...
exports.addProjectView = (req, res) => {
  ClientProject.find({ client: req.params.id })
    .then((clientprojects) => {
...
exports.setPaid = async (req, res) => {
  ClientProject.findOneAndUpdate({ _id: req.params.id }, { paid: true })
    .then((clientproject) => {
...
exports.createInvoice = async (req, res) => {
  let client_project,
    ...
    total = 0
  try {
    client_project = await ClientProject.find({
      client: req.params.id,
      paid: false,
    })
  ...
  ejs.renderFile(
    ...
    (err, data) => {
      if (err) {
        ...
        pdf
          .create(data, options)
          .toFile('invoices/invoice.pdf', function (err, data) {
            if (err) {
              console.log(err)
...
exports.countAttendenceView = async (req, res) => {
  ...
  res.render('countattendence', {
    countattencences: courseattendence,
...
exports.getFinanceHome = async (req, res) => {
  try {
    let history = await Finance.find({})
    ;(balDBBL = 0), (balEBL = 0), (balAB = 0), (balIBBL = 0)
    history.forEach((his) => {
      if (his.type == 'debit') {
        if (his.account == 'DBBL') balDBBL = balDBBL - his.amount
        else if (his.account == 'EBL') balEBL = balEBL - his.amount
        else if (his.account == 'AB') balAB = balAB - his.amount
        else if (his.account == 'IBBL') balIBBL = balIBBL - his.amount
      } else if (his.type == 'credit') {
        if (his.account == 'DBBL') balDBBL = balDBBL + his.amount
        else if (his.account == 'EBL') balEBL = balEBL + his.amount
        else if (his.account == 'AB') balAB = balAB + his.amount
        else if (his.account == 'IBBL') balIBBL = balIBBL + his.amount
...
exports.updateFinance = async (req, res) => {
  let { amount, description, type, date, account } = req.body
  let finance = {
    amount: amount,
    description: description,
    type: type,
  ...
  try {
    let att = await Finance.create(finance)
    req.flash('success', 'Data added successfully!!')
    res.redirect('/dashboard/finance-home')
  } catch (error) {
    console.log(error)
  }
}

exports.financeDelete = async (req, res) => {
  try {
